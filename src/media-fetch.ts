...
export const mediaFetchCache = new Map<string, MediaCacheEntry>();

function pruneMediaFetchCache(): void {
  const now = Date.now();
  for (const [key, entry] of mediaFetchCache) {
    if (now - entry.timestamp > MEDIA_CACHE_TTL_MS || mediaFetchCache.size > MEDIA_CACHE_MAX_ENTRIES) {
      mediaFetchCache.delete(key);
    }
  }
  while (mediaFetchCache.size > MEDIA_CACHE_MAX_ENTRIES) {
    const firstKey = mediaFetchCache.keys().next().value;
    if (firstKey) mediaFetchCache.delete(firstKey);
  }
}
...
export async function fetchRemoteMedia(options: FetchMediaOptions): Promise<FetchMediaResult> {
  ...
  pruneMediaFetchCache();
  const finalCacheKey = url;
  const cached = mediaFetchCache.get(finalCacheKey);
  if (cached) {
  ...
  try {
    ...
    mediaFetchCache.set(finalCacheKey, { buffer, contentType: contentType ?? undefined, timestamp: Date.now() });
